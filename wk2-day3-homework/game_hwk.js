...
// countMoves -> calculates the number of user clicks -> needed to calculate score
const countMoves = () => {
    clicks = n;
    document.getElementById("clicks").firstChild.innerHTML = clicks;
}

// ClearTiles -> Clear tiles when a new game is started;
const clearTiles = () => {
    for (const tile of tiles) {
...
// calculateScore -> adds the number of clicks and elapsed time to calculate the score & displays the score upon game completion.
...
// publish leaderboard;
// use API to generate a random icon or picture
